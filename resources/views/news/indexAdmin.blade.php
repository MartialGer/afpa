...
<div><a href="{{ route('welcome')}}" class="text-red-500 no-underline z-50 align-left">Agenda</a></div>
  <div class="flex items-center justify-center flex-col h-screen gap-10">
    ...
    <div style="background-color: snow;" class="flex items-center justify-center gap-10 h-3/5 flex-col w-4/5 rounded-md">
      <a href="{{ route('news.admin.create')}}" class="flex-start"><x-bouton-ajouter>Ajouter</x-bouton-ajouter></a>
      <table style="background-color: snow;" class="flex gap-10">
        ...
        <tr>
          ...
          <td class="border border-black border-solid rounded-md"><a href="{{ route('news.admin.edit', ['news' => $new->id]) }}"><x-bouton-modif>Modifier</x-bouton-modif></a></td>
          <td class="border border-black border-solid rounded-md">
            <form action="{{ route('news.admin.destroy', ['news' => $new->id]) }}" method="POST">
              @csrf
              @method('DELETE')
